...
def generar_etiquetas_chicas(products_list):
    ...
    for i, (bar_code, name, price, date_str) in enumerate(products_list):
        ...
        c.drawCentredString(x + lbl_w/2, y + 2*mm, f"{bar_code} - {date_str if date_str else label_date}")
    c.save()
    buffer.seek(0)
    return buffer.getvalue()
...
with tab0:
    ...
    if st.session_state.cola_impresion:
        ...
        edited_cola = st.data_editor(df_cola, column_config={"Quitar ❌": st.column_config.CheckboxColumn(default=True)}, disabled=["Código Barra", "Descripción", "Precio", "Fecha", "Tamaño"], hide_index=True, use_container_width=True, key="tabla_viva")
        ...
        if st.session_state.cola_impresion:
            ...
            with cg:
                if lg:
                    ...
                    st.download_button("⬇️ Descargar", data=pdf_g, file_name="gigantes.pdf", use_container_width=True, key="f_g")
                    embeber_e_imprimir_pdf(pdf_g, "p_g")
            with cm:
                if lm:
                    st.write(f"**🔵 MEDIANOS ({len(lm)})**")
                    pdf_m = generar_precios_medianos(lm)
                    st.download_button("⬇️ Descargar", data=pdf_m, file_name="medianos.pdf", use_container_width=True, key="f_m")
                    embeber_e_imprimir_pdf(pdf_m, "p_m")
            with cc:
                if lc:
                    st.write(f"**🟢 CHICOS ({len(lc)})**")
                    pdf_c = generar_etiquetas_chicas(lc)
                    st.download_button("⬇️ Descargar", data=pdf_c, file_name="chicos.pdf", use_container_width=True, key="f_c")
    ...
    if uploaded_file:
        try:
            ...
            for r in reader:
                ...
                parsed_products.append({"Imprimir": True, "SKU": r_ext[2].strip() if r_ext[2].strip() else "S/C", "Descripción": fix_encoding(r_ext[1].strip().strip('"')), "Precio Crudo": r_ext[0].strip(), "Fecha": r_ext[4].strip()})
            ...
            with col1:
                ...
                st.download_button("📥 Bajar Gigantes", data=pdf_csv_g, file_name="carteles_gigantes_a4.pdf", use_container_width=True, key="c_g")
                embeber_e_imprimir_pdf(pdf_csv_g, "csv_p_g")
            with col2:
                pdf_csv_m = generar_precios_medianos(lista_final)
                st.download_button("📥 Bajar Medianos", data=pdf_csv_m, file_name="precios_medianos_10x7.pdf", use_container_width=True, key="c_m")
                embeber_e_imprimir_pdf(pdf_csv_m, "csv_p_m")
            with col3:
                pdf_csv_c = generar_etiquetas_chicas(lista_final)
                st.download_button("📥 Bajar Chicas", data=pdf_csv_c, file_name="etiquetas_chicas_7x35.pdf", use_container_width=True, key="c_c")
    ...
    if file_a and file_b:
        if st.button("Cruzar Listas y Detectar Cambios", type="primary", use_container_width=True):
            try:
                ...
                merged = pd.merge(df_old[["Codigo_Barra", "Precio_num"]].rename(columns={"Precio_num": "Precio_old"}), df_new[["Codigo_Barra", "Precio_num", "Descripcion", "Precio"]], on="Codigo_Barra", how="inner")
        ...
        if "df_comparativa" in st.session_state and not st.session_state.df_comparativa.empty:
            ...
            edited_comp = st.data_editor(
                ...
                column_config={"🖨️ Seleccionar": st.column_config.CheckboxColumn(default=False)},
            ...
            if cant_items > 0:
                ...
                lista_impresion_directa = [(str(row["Código Barra"]), row["Descripcion"], row["Precio_Nuevo"], fecha_hoy) for _, row in df_tildados.iterrows()]
                
                with comp_g:
                    st.write(f"**🔴 Carteles Gigantes ({cant_items})**")
                    pdf_comp_g = generar_carteles_gigantes(lista_impresion_directa)
                    st.download_button("⬇️ Descargar PDF", data=pdf_comp_g, file_name="cambios_gigantes.pdf", use_container_width=True, key="dl_comp_g")
                ...
                with comp_m:
                    ...
                    st.download_button("⬇️ Descargar PDF", data=pdf_comp_m, file_name="cambios_medianos.pdf", use_container_width=True, key="dl_comp_m")
                ...
                with comp_c:
                    ...
                    st.download_button("⬇️ Descargar PDF", data=pdf_comp_c, file_name="cambios_chicas.pdf", use_container_width=True, key="dl_comp_c")
                    embeber_e_imprimir_pdf(pdf_comp_c, "print_comp_c")
            else:
                with comp_g: st.info("🔴 Tildá los ítems a imprimir")
                with comp_m: st.info("🔵 Tildá los ítems a imprimir")
                with comp_c: st.info("🟢 Tildá los ítems a imprimir")
